...
class nucategory_Core {
	...
	public static function form_radio($form_field, array $selected_categories = array(), $columns = 1, $enable_parents = FALSE, $show_hidden = FALSE)
	{
		...
		// Validate columns
		$columns = (int) $columns;
		error_log("kuster@ nu from_radio");
		if ($columns == 0)
		{
			$columns = 1;
		}
		...
		foreach ($category_data as $category)
		{
			...
			// Display parent category.
			$html .= "\n\t".'<li title="'.$category['category_description'].'">';
			$html .= "\n\t\t".nucategory::display_category_radio($category, $selected_categories, $form_field, $enable_parents, $i)."\n";
			...
			if (count($category['children']) > 0)
			{
				...
				{
					$html .= "\n\t\t\t".'<li title="'.$child['category_description'].'">'."\n";
					$html .= nucategory::display_category_radio($child, $selected_categories, $form_field, $enable_parents, $i);
					$html .= "\n\t\t\t".'</li>'."\r\n";
				}
				$html .= "\t\t".'</ul>'."\r\n";
			}
